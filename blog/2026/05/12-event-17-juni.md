---
authors: [tom-ootes]
tags: [meetups, community]
description: |
  Op 17 juni organiseren we een nieuwe developer.overheid.nl meetup in Utrecht. Een middag voor developers, architecten en makers binnen de overheid die willen bouwen, leren en ervaringen uitwisselen.
---

# Kom naar onze meetup op 17 juni: Samen. Beter. Bouwen.

Hoe bouwen we binnen de overheid software die open, veilig, herbruikbaar én
praktisch toepasbaar is?

Op woensdag 17 juni organiseren we een nieuwe developer.overheid.nl meetup in
Utrecht. Een middag voor developers, architecten, platform engineers, tech leads
en makers binnen de overheid die willen bouwen, leren en ervaringen uitwisselen.

Geen beleidsverhalen, maar concrete praktijkvoorbeelden, demo's, open source
tooling en gesprekken met mensen die hier dagelijks aan werken.

<!-- truncate -->

:::success[TL;DR]

We organiseren 17 juni een meetup met hands-on demo's en praktijkvoorbeelden.
Aanmelden kan via
[opensourcewerken.nl](https://opensourcewerken.nl/events/view/28756611-d38b-40c1-8f87-4ac8433831dd/developeroverheidnl-meetup)
of mail naar [v.vanderheijden@geonovum.nl](mailto:v.vanderheijden@geonovum.nl).

:::

## Op het programma

- **Platform engineering** — Anne Schuth  
  Waarom is de Nederlandse overheid misschien wel het grootste softwarebedrijf
  van Nederland. En waarom moeten we ons ook zo organiseren.

- **AI Skill: publiccode.yml generator** — Tom Ootes  
  Hoe je met AI Skills automatisch een publiccode.yml genereert en AI inzet
  binnen je ontwikkelproces.

- **DON-checker** — Dimitri van Hees  
  Live demo van de vernieuwde DON-checker voor het toetsen op standaarden en
  kwaliteit.

- **OpenKAT** — Jan Klopper  
  Een open source securitytool met een actieve community en slimme plugins
  ("boefjes") voor het analyseren van netwerken en systemen.

- **code.overheid.nl** — Johan Groenen  
  Over de gedeelde Forgejo-omgeving voor de overheid en de groeiende behoefte
  aan digitale soevereiniteit en open samenwerking.

- **Open mic & vooruitblik**  
  Welke tooling, standaarden of use cases wil jij de volgende keer zien?

- **Borrel bij Ruig**

## Praktische informatie

|              |                                                                                                                                                                                                                   |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 📅 Datum     | 17 juni 2026                                                                                                                                                                                                      |
| 🕒 Tijd      | 12:30 – 16:00 (aanvang met lunch)                                                                                                                                                                                 |
| 📍 Locatie   | Utrecht, boven het Beatrixtheater                                                                                                                                                                                 |
| ✉️ Aanmelden | Via [opensourcewerken.nl](https://opensourcewerken.nl/events/view/28756611-d38b-40c1-8f87-4ac8433831dd/developeroverheidnl-meetup) of mail naar [v.vanderheijden@geonovum.nl](mailto:v.vanderheijden@geonovum.nl) |

<br/>

<p style={{ fontSize:'1.75rem'}}>

Word contributor.

</p>

We zien je op 17 juni!
