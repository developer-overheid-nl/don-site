...
1.  **Request**: De client stuurt een `POST`-request om een langdurige operatie
    te starten.
2.  **Acceptatie**: De server valideert de aanvraag, slaat de operatie op
    (status "Pending") en stuurt direct een
...
4.  **Statusupdate**: De response toont de huidige status (bijv. "Processing")
    en eventueel een schatting van de resterende tijd.
