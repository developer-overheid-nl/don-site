...
De REST API Design Rules (ADR) schrijven voor waar een REST API van de overheid aan moet voldoen. Een aantal van deze regels is tijdens het API design te testen met de [ADR Linter](./api-design-rules-linter). Als een API eenmaal in productie is kan de [ADR Validator](./api-design-rules-validator) gebruikt worden om bepaalde regels automatisch te valideren.
