...
We gebruiken `@axe-core/webdriverjs` in combinatie met
`scripts/wcag-sitemap-check.js`. Dat script leest de `sitemap.xml` van een
lokaal draaiende site en draait Axe met Selenium en headless Chrome voor elke
URL.
...
- Axe kent geen ingebouwde "crawl" functie; met het script houden we de logica
  klein maar krijgen we toch volledige dekking.
...
- Het Docusaurus‑project draait lokaal op `http://localhost:3000`. Start
  ...
  ```
- Zorg dat `node_modules` up-to-date zijn (`pnpm install`).
  `browser-driver-manager` haalt een bijpassende versie van Chrome en
  ChromeDriver op.
...
- Voor elke pagina voert `@axe-core/webdriverjs` een Axe-analyse uit in dezelfde
  browser.
