---
sidebar_position: 0
---

import Grid from "../../src/components/Grid";
import { Card } from "@rijkshuisstijl-community/components-react";

# Front-end

Front-end development draait om de ontwikkeling van de gebruikersinterface van
websites en webapplicaties.

import DocCardList from '@theme/DocCardList';

## Onderwerpen

<DocCardList />

## Bronnen

- [DigiToegankelijk](https://www.digitoegankelijk.nl/)
- [Gebruiker Centraal](https://www.gebruikercentraal.nl/)
- [NL-Design-System](https://nldesignsystem.nl/)
- [Manon] (https://minvws.github.io/nl-rdo-manon/)
- [Rijkshuisstijl Community](https://www.rijkshuisstijl-community.nl/)

## Aan de slag!

<Grid>
  <Card
    appearance="default"
    description="Wil je snel aan de slag met NL Design System? Volg dan deze tutorial."
    heading="Getting started: NL Design System"
    href="./design-systems/nl-design-system/aan-de-slag-met-nl-design-system"
    linkLabel="Naar de tutorial"
  />

  <Card
    appearance="default"
    description="Wil je snel bouwen met rijkshuisstijl componenten? Volg dan deze tutorial."
    heading="Getting started: Rijkshuisstijl Community"
    href="https://rijkshuisstijl-community.vercel.app/?path=/docs/react-components-readme--docs"
    linkLabel="React componenten docs"
  />

  <Card
    appearance="default"
    description="Wil je snel aan de slag met het MinVWS Manon Design System? Volg dan deze tutorial."
    heading="Getting started: manon"
    href="./design-systems/manon/aan-de-slag-met-manon"
    linkLabel="Naar de tutorial"
  />

  <Card
    appearance="default"
    description="Monitor je project continu op de WCAG-toegankelijkheidseisen."
    heading="Voeg een Github action toe voor accessibility"
    href="./standaarden/digitoegankelijk/run-axe"
    linkLabel="Voeg een Github action toe"
  />

  <Card
    appearance="default"
    heading="Werken met geodata en LeafletJS"
    description="Gemeente Amsterdam biedt een developer omgeving met code examples."
    href="./tools/maps-developers-amsterdam"
    linkLabel="Naar het artikel"
  />

  <Card
    appearance="default"
    heading="Werken met formulieren en React Hook Form"
    description="Gemeente Amsterdam biedt een developer omgeving met code examples."
    href="./tools/forms-developers-amsterdam"
    linkLabel="Naar het artikel"
  />
</Grid>

<br />
<br />
